...
auto init_thermo_mcmc(std::size_t n_walkers, std::size_t n_beta,
  ...
  ensemble<by_beta<mcmc<Parameters>>> walker(n_walkers,
                                             by_beta<mcmc<Parameters>>(n_beta));
...
concept is_Algorithm_conditions = requires(Algorithm &&a) {
  {
    checks_convergence(std::move(a), std::declval<const thermo_mcmc<Parameters> &>())
...
public:
  less_than_max_iteration(std::size_t max_iter)
      : current_iteration{0ul}, max_iter_{max_iter} {}

  less_than_max_iteration &operator++() {
    ++current_iteration;
    return *this;
  }
  operator bool() { return current_iteration < max_iter_; }
  template <class Anything>
  friend auto checks_convergence(less_than_max_iteration&& c, const Anything &) {
    if (c) {
      ++c;
...
auto &step_stretch_thermo_mcmc(const by_beta<double> &beta,
                               ...
                               thermo_mcmc<Parameters> &current, Model &model,
                               const IndexedData &y, const Variables &x,
...
#pragma omp parallel for
    for (std::size_t i = 0; i < n_walkers / 2; ++i) {
      ...
      for (std::size_t ib = 0; ib < n_beta; ++ib) {
        ...
        auto dthLogL = ca_logP - current.walkers[iw][ib].logP +
...
auto push_back_new_beta(thermo_mcmc<Parameters> &current,
                        ensemble<std::mt19937_64> &mts, Model &model,
                        const IndexedData &y, const Variables &x) {
...
auto thermo_impl(const Algorithm &alg, Model &model, const IndexedData &y,
                 const Variables &x, std::size_t num_scouts_per_ensemble,
  ...
  auto current = init_thermo_mcmc(n_walkers, beta_run.size(), mts, model, y, x);
  ...
  while (beta_run.size() < beta.size() || !mcmc_run.second) {
    ...
    if (beta_run.size() < beta.size()) {
      ...
      current = push_back_new_beta(current, mts, model, y, x);
      mcmc_run = checks_convergence(std::move(mcmc_run.first), current);
    }
  }
...
template <class Model, class Variables, class IndexedData,
          class Parameters = std::decay_t<decltype(sample(
              std::declval<std::mt19937_64 &>(), std::declval<Model &>()))>>
  requires(is_model<Model, Parameters, Variables, IndexedData>)
    auto thermo_max_iter(Model model, const IndexedData &y, const Variables &x,std::size_t max_iter,
                     std::size_t num_scouts_per_ensemble, double jump_factor,
                     double stops_at, bool includes_zero,
                     std::size_t initseed) {
  return thermo_impl(less_than_max_iteration(max_iter),model,  y, x,
                     num_scouts_per_ensemble, jump_factor, stops_at,
