...
class independent_variable_model{
 ...
 public:
  independent_variable_model(  std::size_t npar = 40ul,
                                 double log10_std_par = 1.0,

                                 double mean_mean_par = 0.0,
                                 double std_mean_par = 10.0
):
          npar_{npar},log10_std_par_{log10_std_par},mean_mean_par_ {mean_mean_par},
  ...
  auto operator()(std::mt19937_64 &mt, std::size_t nsamples)
    ...
    auto par_stds = apply([](auto &x) { return std::pow(10, x); },
                          sample(mt,normal_distribution( 0, log10_std_par_), 1, npar_));

    auto cov_par = covariance_model(npar_, par_stds)(mt);
    auto mean_par =
        sample(mt,normal_distribution( mean_mean_par_, std_mean_par_), 1, npar_);

    auto par_dist_ = make_multivariate_normal_distribution(mean_par,
                                                           cov_par);
...
class bayesian_linear_model
  ...
  dist_type const& prior()const {return *this;}
  linear_model const & likelihood()const {return *this;}
};
...
auto make_bayesian_linear_model(double prior_eps_df,
  ...
  return       make_bayesian_linear_model(
...
template <class Cova>
    requires Covariance<double, Cova>
auto evidence(conjugate,
    const multivariate_gamma_normal_distribution<double, Cova> &prior,
    const linear_model& li,
    const Matrix<double> &y,
    const Matrix<double> &X) {
...
auto bayesian_linear_regression_calculate_var_logLik_diff(
  ...
  return (bayesian_linear_regression_calculate_mean_logLik(prior,y,X,beta1)-
          bayesian_linear_regression_calculate_mean_logLik(prior,y,X,beta0))/(beta1-beta0);
 }
